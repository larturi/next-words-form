...
const Form: React.FC<FormProps> = ({ isEnabled }) => {
  ...
  const validationForm = () => {
    ...
    if (wordFormsValue) {
      validateInput('v', verb, setVerbStateInput, setVerbResults, wordFormsValue)
      validateInput('n', noun, setNounStateInput, setNounResults, wordFormsValue)
      validateInput('a', adjective, setAdjectiveStateInput, setAdjectiveResults, wordFormsValue)
      validateInput('r', adverb, setAdverbStateInput, setAdverbResults, wordFormsValue)
    }
  }
  ...
  const validateInput = (
    ...
    if (wordFormsValue?.word_forms[wordType].length > 0) {
      if (wordFormsValue.word_forms[wordType].includes(word.toLowerCase().trim())) {
                  ...
                  {wordFormsValue && !errorService ? (
                    ...
                    <>
                      <p className='text-red-400 text-sm lg:pr-20'>
                        We are experiencing issues with our word service. Please try again later.
                      </p>
                    </>
                  ) : (
                    '...'
              ...
              <div className='flex flex-col gap-4 mt-4'>
                ...
                {isSubmited && (
                  <div className='text-sm ml-1 -mt-3 text-gray-300'>
                    {verbResults !== '' ? verbResults : `${randomWordValue} do not have verb forms`}
                  </div>
                )}
                ...
                {isSubmited && (
                  <div className='text-sm ml-1 -mt-3 text-gray-300'>
                    {nounResults !== '' ? nounResults : `${randomWordValue} do not have noun forms`}
                  </div>
                )}
